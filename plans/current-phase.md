...
## Phase Overview
**Phase**: Launch — **1.1.0 (build 6) SUBMITTED for public App Store review 2026-08-17**, manual release selected; paywall (1.2.0) planned behind it
**Status**: 🚀 **Submitted.** Build **6** (v1.1.0, commit `0ec008a4`, EAS build `ba77af13`) uploaded via submission `72386e8b` and sent to App Review 2026-08-17 with **manual release** — approval puts the release button in Tui's hand; nothing goes public until the 1.2.0-paywall decision (current lean: hold and launch paid, see `plans/paywall-1.2.0.md`). Build 6 was cut for two reasons no OTA could carry to a reviewer's first launch: a real `NSMicrophoneUsageDescription` (PR #65 — expo-audio's generic default was a 5.1.1 magnet for a voice-first app) and a current embedded bundle (build 5's was `454e987`, predating education, threads, and the delete fix). Submission-day work: **App Privacy label** filled (7 types — Name, Email, Precise Location, User ID, Device ID, Other User Content, Search History; all linked-to-user, none tracking, App Functionality only; **Audio Data deliberately not declared** — recordings are transcribed and discarded, only transcripts persist); ASC version record renamed 1.0 → 1.1.0 (build picker was empty against every 1.1.0 binary); review notes pasted plain-text with credentials only in the Sign-In fields; **demo account re-seeded** (PR #66 — the Griffith open note had been accidentally deleted during feature testing; reviewer would have seen nothing; verifier snippet now in `docs/APP_STORE_REVIEW_NOTES.md`, note `1bc3e058…`); **delete-account flow submitted for real** (PR #67 — `ACCOUNT_DELETE_START→COMPLETE` in Railway logs, 176 ms; 5.1.1(v) covered; Weaviate purge path still unexercised, needs a throwaway with an embedded note); log-out sheet no longer leaks build identity (PR #70, moved to Settings footer). Both OTA channels serve merge `161d1ac` — `preview` group `6c4a445f`, `production` group `01f0a5d6`. Earlier 08-17: notes delete-sheet fix (PR #64, device-verified incl. Places).
**Up Next After Launch**: **1.2.0 subscription paywall** — plan merged at `plans/paywall-1.2.0.md` (PRs #68–#69). Pricing settled: **$4.99/mo, $49.99/yr ("2 months free"), 14-day Introductory Offer**; app itself stays Free in ASC. Start now (lead time): Paid Applications Agreement + Small Business Program. Then Phase 7 (Cross-Domain Synthesis & AI Insights).
**Previous Phase**: 5 & 6 (Semantic Intelligence + Geofencing) - ✅ Complete
**Current Date**: 2026-08-17
**Last Updated**: 2026-08-17 — **Ask Offload threads shipped as standing queries** (backend + mobile, JS-only so it rides OTA). Ask Offload looked like a chat and was not one: the client held messages in React state (dead on unmount) and `/rag/spar` sent a single turn with no history, so even a live follow-up was answered blind. Both are fixed, but the design deliberately is *not* a saved ChatGPT thread — a replayed transcript is a record of what was true then, and reopened weeks later it confidently repeats a stale answer. A thread now stores the **opening question**, the **object ids its answers stood on**, and a **watermark**, and re-derives on resume: reopening auto-reports what resolved, what's still open, what was deleted, and what's been captured since (a fresh semantic sweep on the opening query, not the latest message). Buckets are computed deterministically in SQL/code — the LLM only narrates what it is handed, same division of labour as `synthesisService`'s Accomplished list. Pieces: migration `023_conversations.sql`, `models/Conversation.ts`, `services/conversationService.ts`, `routes/conversations.ts` (`/api/v1/conversations`, `GET /:id` side-effect free, `POST /:id/resume` does the diff), multi-turn + thread-summary compaction in `sparringService.ts`, and mobile `useAI` rewritten thread-aware with a saved-threads sheet and a distinct delta card in `AIQueryScreen`. Also fixed along the way: cited ids are now intersected with what was actually retrieved, so a hallucinated citation can't inflate the "N notes cited" footer. 29 new backend tests (560 total green); mobile `tsc` still has the same 3 pre-existing errors. **Not yet device-verified — needs an `eas update` and a real reopen-after-days test.**

**Previously (2026-08-13, late)** — Progressive onboarding + How Offload Works shipped and device-verified (PRs #59–#61: education system, Ask Offload keyboard fix + notes-lifecycle module, "© AIGA LLC" in mobile Settings and web footer; both OTA channels serve merge `26c8d8b`, Vercel redeployed). Also re-verified: the weekly-digest HST gap is **closed** — PR #50 (`e1f2c44`) fires it Sunday 18:00 in each user's `last_seen_timezone`, HST fallback only for pre-capture accounts; CLAUDE.md's "still HST-fixed" claim was stale and has been corrected. Monthly synthesis still runs on server-time "1st of the month" (month-granularity, accepted). Earlier same day: time reminders rebuilt — accurate to the second, fire from the device, break through Focus (PRs #51–#52, session log at bottom). Build 5 submitted.
...
- ~~**Onboarding / "what is Offload" content**~~ — ✅ **built 2026-08-13** as a progressive education system (JS-only, OTA-able). Pieces: (1) `IntroScreen` — 3 swipeable first-run cards, **new signups only** (flag set in `register()`), shown *before* the permission ladder so the asks land with context; (2) `HowOffloadWorksScreen` — permanent guide under Settings → About (talk naturally / places + live arrival-permission state / time reminders / tappable Ask Offload examples / Insights with CTA); (3) `TrySayingSheet` — "Try saying…" pill on the idle Record screen; (4) `FirstRecordingSheet` on Home — after a new signup's first recording is sorted, shows the *actual* parsed notes ("One recording. N separate memories."), deferred while `ArrivalPermissionSheet` is up; (5) existing accounts get **no forced onboarding** — one discovery toast pointing at the guide. State in `educationService.ts` (SecureStore flags, reset on sign-out/delete like the permission flags; fails closed). Also stood up mobile's first test infra: minimal jest + ts-jest for pure service logic (`npm test`, 10 tests) — not jest-expo, which remains its own piece of work. Note: `tsc --noEmit` has 3 pre-existing errors (`api.ts:351`, `locationService.ts:307`, `websocket.ts:167`) unrelated to this work.
