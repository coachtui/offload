...
- [ ] **Step 1: Write the failing arbitration tests** from the captured API data. These four cases are the specification:
  ...
  - `Costco` (20 rows incl. gas station, food court, tire centre, pharmacy, optical, bakery sharing warehouse addresses) → `chain` with **5** locations, and every surviving name is "Costco Wholesale", never "Costco Gasoline".
  - `[]` → `none`.
- [ ] **Step 2: Implement `arbitrate`** in three ordered stages — (a) name filter: keep exact match or query-as-prefix on `displayName`; (b) co-location collapse: rows within 300 m of each other become one, keeping the member whose name is closest to the query; (c) count → `single` (1) / `ambiguous` (2) / `chain` (≥3).
- [ ] **Step 3: Write the failing anchor tests** — a note naming a region ("the Foodland in Hilo") anchors on Hilo; a mainland recording with no named region anchors on the home centroid; a user with no history yields only the current-location anchor.
- [ ] **Step 4: Implement `resolveAnchors`** in priority order: region named in the note → current location → home region. Home region = centroid of the user's manual geofences and places, falling back to the modal location of their last ~50 sessions. Cache on `hub.users` (add `home_lat`/`home_lng`/`home_computed_at` in Task 4's migration), recompute when older than 7 days.
- [ ] **Step 5: Chain the providers** in `resolvePlaceNameMulti` — for each anchor in order, try OSM then Google; first anchor yielding candidates wins. Short-circuit, never merge providers.
- [ ] **Step 6: Replace the hard bound with the distance gate** — drop a candidate further than 100 km from *every* anchor. Keep `bounded=1` per-anchor inside `osmProvider` (removing it is what returned McDonald's in Spain).
- [ ] **Step 7: Verify** — `npx tsc --noEmit`, `npm test`. Existing `placeMatching.test.ts` and `reapInferredGeofences.test.ts` must stay green.
